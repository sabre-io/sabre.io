...
* We finally have support for CalDAV scheduling, as defined in
  ...
  people on your sabredav server for CalDAV servers, and see attendence
  status automatically update. Many CalDAV clients support this, and will
  also now automatically provide "Accept" and "Decline" interfaces when new
  invites are sent. Read more about this in the
  [scheduling documentation][scheduling].
* Support for sending event invitations for every CalDAV client. See
  [imip][imip] for more info.
...
* We also support vCard 4.0 everywhere! If you don't request a specific
  ...
  convert in between these formats on request.
...
If you use the default `PDO` backend for CalDAV in specific, you need to
migrate to the lastest database structure.
...
created a custom backend, you must add support for this.

If you extended `Sabre\CalDAV\Backend\AbstractBackend`, a fallback has been
provided though, although this fallback is rather slow.


### Renamed classes

We have renamed a few classes to be more consistent:
...
has been replaced by a new [iMip plugin][imip]. A benefit of this plugin, is
that it works for every caldav client, and not just broken iCal versions.


### Support for free-busy reports has moved
...
If you implemented your own `PropertyStorage` class, you must now implement
the new `move` method.
